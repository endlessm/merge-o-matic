...
class FindUpstreamTest(unittest.TestCase):
    def setUp(self):
        self.target_repo, self.source_repo = th.standard_simple_config()
    ...
    def test_sourceIsNewer(self):
        ...
        th.build_and_import_simple_package('foo', '2.0', self.source_repo)
...
class FindUnstableUpstreamTest(unittest.TestCase):
    ...
    def test_upstreamFromUnstable(self):
        th.build_and_import_simple_package('foo', '2.3', self.target_repo)
    ...
    def test_upstreamFromFirstUnstable(self):
        th.build_and_import_simple_package('foo', '2.1', self.target_repo)
    ...
    def test_upstreamFromSecondUnstable(self):
        th.build_and_import_simple_package('foo', '2.6', self.target_repo)
        th.build_and_import_simple_package('foo', '2.0',
                                           self.stable_source_repo)
        th.build_and_import_simple_package('foo', '2.4',
                                           self.unstable1_source_repo)
...
class HandlePackageTest(unittest.TestCase):
    ...
    def test_unmodifiedUpgrade(self):
        th.build_and_import_simple_package('foo', '1.0', self.target_repo)
        th.build_and_import_simple_package('foo', '2.0', self.source1_repo)
    ...
    def test_mergeUpgrade(self):
        th.build_and_import_simple_package('foo', '1.0-1mom1',
                                           self.target_repo)
        th.build_and_import_simple_package('foo', '1.0-1',
                                           self.source1_repo)
        th.build_and_import_simple_package('foo', '2.0-1',
                                           self.source2_repo)
        target = config.targets()[0]
        th.update_all_distro_sources()
        pv = target.distro.findPackage('foo', version='1.0-1mom1')[0]
